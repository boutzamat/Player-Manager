        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            <tr class="<?php echo $i % 2 === 0 ? 'odd' : 'even'; ?>">
                <td class = " <?php echo ($i == 0) ? "first" : "ajax_item_name group"; ?>" axis="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminSettings&amp;action=updateGroup&amp;id=<?php echo @$tpl['group_arr'][$i - 1]['id']; ?>">
                    ...
                    if ($i == 0) {
                        ?>
                    <input type="text" class="text-all" name="group_name" value="<?php echo $tpl['group']['group_name']; ?>"/>
                        <?php
                    } else {
                ...
                if ($i == 0) {
                    ?>
                    <td>
                        <a class="icon icon-edit-group ajax-update-group" href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminSettings&amp;action=editGroup&amp;id=<?php echo $tpl['group']['id']; ?>"><?php echo $CMS_LANG['_save']; ?></a>
                    </td>
                    <?php
                } else {
                    ?>
