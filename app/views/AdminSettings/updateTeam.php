...
if (isset($tpl['status'])) {
	...
	<h1 class="page-header"><?php echo $CMS_LANG['menu_settings']; ?></h1>	
    ...
    <div id="teamsTab"> <!-- id="tabs" class="tab-user-general" --> 
		<ul class="nav nav-tabs" role="tablist">
			<li class="active" role="presentation"><a href="#tabs-1" aria-controls="settings_teams" role="tab" data-toggle="tab"><?php echo $CMS_LANG['settings_teams']; ?></a></li>
			<li role="presentation"><a href="#tabs-1" aria-controls="settings_groups" role="tab" data-toggle="tab"><?php echo $CMS_LANG['settings_groups']; ?></a></li>
		</ul>
							...
							for ($i = 0; $i < $count; $i++) {
								...
								<tr class="<?php echo $i % 2 === 0 ? 'odd' : 'even'; ?>">
									<td class = "<?php echo ($i == 0) ? "first" : "ajax_item_name"; ?>" axis="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminSettings&amp;action=updateTeam&amp;id=<?php echo @$tpl['team_arr'][$i - 1]['id']; ?>">
										...
										if ($i == 0) {
											?>
											<input type="text" class="text-all form-control" name="team_name" value="<?php echo $tpl['team']['team_name'] ?>"/>
											<?php
										} else {
									...
									<td class = " <?php echo ($i == 0) ? "first" : "ajax_item_name"; ?>" axis="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminSettings&amp;action=updateTeam&amp;id=<?php echo @$tpl['team_arr'][$i - 1]['id']; ?>">
										...
										if ($i == 0) {
											?>
											<input type="text" class="text-all form-control" name="number_from" value="<?php echo $tpl['team']['number_from'] ?>"/>
											<?php
										} else {
									...
									<td class = " <?php echo ($i == 0) ? "first" : "ajax_item_name"; ?>" axis="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminSettings&amp;action=updateTeam&amp;id=<?php echo @$tpl['team_arr'][$i - 1]['id']; ?>">
										...
										if ($i == 0) {
											?>
											<input type="text" class="text-all form-control" name="number_to" value="<?php echo $tpl['team']['number_to'] ?>"/>
											<?php
										} else {
									...
									if ($i == 0) {
										?>
										<td>
											<a class="icon icon-edit ajax-update-team btn btn-sm btn-default" href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminSettings&amp;action=editTeam&amp;id=<?php echo $tpl['team']['id']; ?>"><?php echo $CMS_LANG['_save']; ?></a>
										</td>
										<?php
									} else {
										?>
