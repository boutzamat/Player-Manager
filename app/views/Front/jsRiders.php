 ...
 */ ?>
<div id="content" class="content-middle">
    <div class="notice-box">
        <div class="notice-top"></div>
        <div class="notice-middle"><span class="notice-info"> </span><?php echo $tpl['option_arr']['info_box']; ?></div>
        <div class="notice-bottom"></div>
    </div>
    <form id="frmSearch" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="frmSearchName" class="cms-form b10">
        <span>
            <input type="text" name="search_str" value="<?php echo isset($_GET['search_str']) && !empty($_GET['search_str']) ? htmlspecialchars($_GET['search_str']) : NULL; ?>" class="text w300" id="search_str_id" />
            <input type="submit" value="" class="button button_search" id="serach_btn_id" name="serach_btn_name"/>
        </span>

        <select name="sort" class="select select-margin" id="sort_id">
            ...
            <ul class="cms-paginator">
                ...
                for ($i = 1; $i <= $tpl['paginator']['pages']; $i++) {
                    if ((isset($_GET['page']) && (int) $_GET['page'] == $i) || (!isset($_GET['page']) && $i == 1)) {
                        ?><li><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=<?php echo $_GET['controller']; ?>&amp;action=index&amp;search_str=<?php echo isset($_REQUEST['search_str']) && !empty($_REQUEST['search_str']) ? urlencode($_REQUEST['search_str']) : NULL; ?>&amp;sort=<?php echo isset($_REQUEST['sort']) && !empty($_REQUEST['sort']) ? urlencode($_REQUEST['sort']) : NULL; ?>&amp;filter=<?php echo isset($_REQUEST['filter']) && !empty($_REQUEST['filter']) ? urlencode($_REQUEST['filter']) : NULL; ?>&amp;page=<?php echo $i; ?>" class="pages focus" rel="<?php echo $i; ?>"><?php echo $i; ?></a></li><?php
            } else {
                        ?><li><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=<?php echo $_GET['controller']; ?>&amp;action=index&amp;search_str=<?php echo isset($_REQUEST['search_str']) && !empty($_REQUEST['search_str']) ? urlencode($_REQUEST['search_str']) : NULL; ?>&amp;sort=<?php echo isset($_REQUEST['sort']) && !empty($_REQUEST['sort']) ? urlencode($_REQUEST['sort']) : NULL; ?>&amp;filter=<?php echo isset($_REQUEST['filter']) && !empty($_REQUEST['filter']) ? urlencode($_REQUEST['filter']) : NULL; ?>&amp;page=<?php echo $i; ?>" class="pages" rel="<?php echo $i; ?>"><?php echo $i; ?></a></li><?php
            }
        }
                ?>
            </ul>
